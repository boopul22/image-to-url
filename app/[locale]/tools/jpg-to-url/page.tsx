...
export async function generateMetadata({
  ...
  const toolDict = dict.tools?.jpgToUrl

  return {
    title: toolDict?.meta?.title || "JPG to URL Converter - Free Online Tool | ImageToURL",
    description: toolDict?.meta?.description || "Convert JPG images to shareable URLs instantly. Free online JPG to URL converter with no signup required. Upload JPEG files up to 10MB and get instant links.",
    keywords: toolDict?.meta?.keywords || "jpg to url, jpeg to url, convert jpg to url, jpg image to url converter, jpg link generator",
    alternates: {
      canonical: `/${locale}/tools/jpg-to-url`,
    },
    openGraph: {
      title: toolDict?.meta?.title || "JPG to URL Converter - Free Online Tool",
      description: toolDict?.meta?.description || "Convert JPG images to shareable URLs instantly.",
...
export default async function JpgToUrlPage({
  ...
  const { locale } = await params
  const dict = await getDictionary(locale)
  const toolDict = dict.tools?.jpgToUrl || defaultToolContent
